...
// Orden e iconos de los servicios (los textos vienen del diccionario, mismo orden)
const SERVICIO_META = [
  { key: 'acompanamiento-medico', icon: IconSalud },
  { key: 'tramites', icon: IconTramites },
  { key: 'notaria-gestoria', icon: IconNotaria },
  { key: 'compraventa-propiedad', icon: IconPropiedad },
  { key: 'banca', icon: IconBanca },
  { key: 'interpretacion-telefonica', icon: IconTelefono },
  { key: 'preparacion-entrevista', icon: IconEntrevista },
  { key: 'clases-espanol', icon: IconEspanol },
];
...
export default async function ServiciosPage() {
  ...
  const t = dict.serviciosPage;
  const SERVICIOS_DEF = SERVICIO_META.map((m, i) => ({ ...m, ...t.items[i] }));
    ...
    <div className="min-h-screen" style={{ background: 'var(--bone)' }}>
      ...
      <div className="max-w-4xl mx-auto px-6 py-16 space-y-12">
        {SERVICIOS_DEF.map((srv) => {
          ...
          const directorioHref = localePath(locale, catId ? `/directorio?categoria=${catId}` : '/directorio');
          return (
            <div
              key={srv.key}
            ...
            >
              ...
              <div className="flex-1">
                ...
                >
                  {t.verAcompanantes.replace('{tema}', srv.titulo)}
