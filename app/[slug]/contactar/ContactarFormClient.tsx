...
export function ContactarFormClient({ slug, locale, t }: Props) {
  const tc = t.contactar;
    ...
    <div className="min-h-screen bg-(--bone)">
      <div className="max-w-xl mx-auto px-4 py-12">
        ...
        <div className="mb-6 text-sm text-(--ink)/50 space-x-2">
          ...
          <span>›</span>
          <span className="text-(--ink)/80">{tc.breadcrumb}</span>
        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label className={labelClass}>
                  {tc.nombre} <span style={{ color: 'var(--terra)' }}>*</span>
                </label>
                <input
                  name="nombre"
                  type="text"
              ...
              <div>
                <label className={labelClass}>
                  {tc.email} <span style={{ color: 'var(--terra)' }}>*</span>
                </label>
                <input
                  name="email"
                  type="email"
              ...
              <div>
                <label className={labelClass}>
                  {tc.mensaje} <span style={{ color: 'var(--terra)' }}>*</span>
                </label>
                <textarea
                  name="mensaje"
                  required
                  rows={5}
                  placeholder={tc.mensajePlaceholder}
